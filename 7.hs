...
runProgsA prog (arg:rest) val = runProgsA prog rest val'
	where
		(_, _, _, outp) = icrun (0, prog, [arg, val], id)
		[val'] = outp []
...
runProgsB prog args init = last $ outputs !! (num - 1)
	where
		num = length args
		machines = [ (0, prog, (arg:) $ (if n == 0 then (init:) else id) $ outputs !! ((n - 1) `mod` num), id) | (n, arg) <- enumerate args ]
		outputs = [ (\(_, _, _, outp) -> outp []) $ icrun machine | machine <- machines ]
