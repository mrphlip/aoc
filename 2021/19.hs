...
parseScanner :: String -> [Vec]
parseScanner = map parsePoint . filter ((/="---").take 3) . lines
...
tryMergeAlign :: [Vec] -> [Vec] -> Alignment -> Maybe [Vec]
tryMergeAlign s1 s2 a = if length (s1 `intersect` transformed) >= minOverlap then Just (s1 `union` transformed) else Nothing
	where transformed = map (transformVec a) s2

tryMergeScanners :: [Vec] -> [Vec] -> Fingerprint -> Fingerprint -> Maybe [Vec]
tryMergeScanners s1 s2 f1 f2 = foldr (<|>) Nothing $ map (tryMergeAlign s1 s2) $ alignScanner s1 s2 f1 f2

tryMerge :: [[Vec]] -> [Fingerprint] -> (Integer, Integer) -> Maybe [[Vec]]
...
oneMerge scanners = fromJust $ foldr (<|>) Nothing $ map (tryMerge scanners fingerprints) $ chooseMerge fingerprints
	where fingerprints = map fingerprint scanners
...
tests :: IO ()
tests = do
	check $ (length $ fullMerge sample) == 79
	where
...
main = do
	tests
	scanners <- getInput
	print $ length $ fullMerge scanners
